...
sudo -u $WEB_USER $PYTHON_ENV_PATH/bin/gunicorn --daemon --name $APPLICATION_NAME --bind unix:$RUN_PATH/backend.sock --pid $RUN_PATH/backend.pid --log-file $LOG_PATH/backend.log wsgi:app
