...
sudo -u $WEB_USER $PYTHON_ENV_PATH/bin/gunicorn --name $APPLICATION_NAME --bind unix:/$RUN_PATH/backend.sock --daemon --pid $RUN_PATH/backend.pid --error-logfile $LOG_PATH/backend.log wsgi:app
